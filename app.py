...
<style>
    ...
    .q-card { background: #F1F5F9; border-left: 4px solid #6366F1; padding: 16px; margin-bottom: 12px; border-radius: 0 8px 8px 0; }
    .stButton > button { width: 100%; background-color: #6366F1 !important; color: white !important; border-radius: 8px !important; font-weight: bold !important; }
</style>
...
if file_source:
    if st.session_state.last_file != file_source.name:
        st.session_state.summary_cache = ""; st.session_state.keywords_cache = []; st.session_state.question_cache = []
    ...
    if module == "Executive Summary":
        if st.button("🚀 EXECUTE SUMMARY"):
            with st.status("Analyzing...") as status:
                ...
                except: st.info("Notice: Optimized summary for short documents.")
        ...
        if st.session_state.summary_cache:
            ...
            st.download_button(label="📥 DOWNLOAD REPORT", data=pdf_gen.output(dest='S').encode('latin-1'), file_name="Summary.pdf")
        ...
        if st.button("🔍 ANALYZE & GENERATE QUESTIONS"):
            with st.spinner("Generating insights..."):
                try:
                    ...
                    if len(subjects) < 10:
                        subjects += ["the core content", "the strategic plan", "operational logic", "data trends", "system architecture"]
                    
                    templates = ["What are the primary objectives of {}?", "How does the report measure {}?", "What risks are associated with {}?", "What is the methodology behind {}?", "What are the future implications of {}?"]
                    
                    # Loop using safe indexing
                    st.session_state.question_cache = [templates[i % 5].format(subjects[i]) for i in range(min(10, len(subjects)))]
                except Exception:
                    st.info("The AI is refining the question set based on document length.")
