    ...
    if module == "Executive Summary":
        ...
        if st.button("🔍 ANALYZE & GENERATE QUESTIONS"):
            with st.spinner("Generating insights..."):
                ...
                if len(subjects) < 10: subjects += ["the project", "the strategic plan", "the audit results", "operational risk", "financial growth"]
        ...
        st.markdown(f'<div class="content-card">This document has <b>{total_pages}</b> pages.</div>', unsafe_allow_html=True)
        ...
        if st.button("✂️ SPLIT & EXPORT"):
            ...
            st.download_button("📥 DOWNLOAD SPLIT PDF", io.BytesIO(writer.write_stream()).getvalue(), "Split_Document.pdf")
